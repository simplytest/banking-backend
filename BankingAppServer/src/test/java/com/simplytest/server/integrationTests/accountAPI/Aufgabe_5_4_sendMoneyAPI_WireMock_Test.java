...
public class Aufgabe_5_4_sendMoneyAPI_WireMock_Test {
    ...
    @Test
    @DisplayName("IBAN-Validierung schlägt fehl: Validator antwortet mit 403 Forbidden")
    public void sendMoneyWithWireMockForbiddenTest() {

        long accountID = dbContract.value().getAccounts().entrySet().stream().filter(x -> x.getValue().getType() == AccountType.GiroAccount).findFirst().get().getKey().child();
        SendMoney sendPayload = new SendMoney(new Iban("DE45120300000000202053"), 250.0);
        String body = Json.get().toJson(sendPayload);

        // WireMock antwortet mit 403 Forbidden → IBAN-Validierung schlägt fehl
        WireMock.stubFor(WireMock.get("/validator/validate?iban=" + sendPayload.target().raw())
                .willReturn(WireMock.forbidden()));

        String url = String.format(Locale.US, "/api/accounts/%d/sendexternal", accountID);
        var responseSend = restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(body, headers), String.class);
        System.out.println("Send response (forbidden): " + responseSend.getBody());

        // Erwartung: 400 Bad Request und Fehler BadIban, da Validierung fehlgeschlagen
        var sendResult = Json.get().fromJson(responseSend.getBody(), new TypeToken<Result<Boolean, com.simplytest.core.Error>>() {});
        Assertions.assertEquals(HttpStatusCode.valueOf(400), responseSend.getStatusCode());
        Assertions.assertEquals(com.simplytest.core.Error.BadIban, sendResult.error());

        // Kontostand muss unverändert sein, da keine Überweisung stattgefunden hat
        double balance = getBalanceForAccount(accountID);
        Assertions.assertEquals(initialBalance, balance);
    }

    @Test
    @DisplayName("IBAN-Validierung schlägt fehl: Validator antwortet mit 500 Internal Server Error")
    public void sendMoneyWithWireMockServerErrorTest() {

        long accountID = dbContract.value().getAccounts().entrySet().stream().filter(x -> x.getValue().getType() == AccountType.GiroAccount).findFirst().get().getKey().child();
        SendMoney sendPayload = new SendMoney(new Iban("DE45120300000000202053"), 250.0);
        String body = Json.get().toJson(sendPayload);

        // WireMock antwortet mit 500 Internal Server Error → IBAN-Validierung schlägt fehl
        WireMock.stubFor(WireMock.get("/validator/validate?iban=" + sendPayload.target().raw())
                .willReturn(WireMock.serverError()));

        String url = String.format(Locale.US, "/api/accounts/%d/sendexternal", accountID);
        var responseSend = restTemplate.exchange(url, HttpMethod.POST, new HttpEntity<>(body, headers), String.class);
        System.out.println("Send response (server error): " + responseSend.getBody());
